...
const char *server_introspection_xml =
	...
	"  </interface>\n"
  	...
  	"  <interface name='local.qdbuscpp2xml.MyLight'>\n"
    	"    <property name='isOn' type='b' access='readwrite'/>\n"
 	...
 	"  </interface>\n"
  	...
  	"  <interface name='local.qdbuscpp2xml.DiagController'>\n"
    	"    <method name='diagRequest'>\n"
    	"    </method>\n"
 	"  </interface>\n"
...
DBusHandlerResult server_message_handler(DBusConnection *conn, DBusMessage *message, void *data)
{
	...
	if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE, "Introspect")) {
 	...
 	/* My diagRequest()/printf() implimentation **************************************************/
	}  else if (dbus_message_is_method_call(message, "local.qdbuscpp2xml.DiagController", "diagRequest")) {
		const char *pong = " 30000500000000 ";
...
int main(void)
{
	...
	rv = dbus_bus_request_name(conn, "org.BDiag.DiagServer", DBUS_NAME_FLAG_REPLACE_EXISTING , &err);
	if (rv != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		fprintf(stderr, "Failed to request name on bus: %s\n", err.message);
		goto fail;
	}

	if (!dbus_connection_register_object_path(conn, "/org/BDiag/DiagControl", &server_vtable, NULL)) {
		fprintf(stderr, "Failed to register a object path for 'DiagControl'\n");
		goto fail;
	}
	 ...
	 * used.
	 */
	printf("Starting dbus DiagServer v%s\n", version);
